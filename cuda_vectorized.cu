...
__global__ void matmul_a_b_kernel(float *A, float *B, float *C,
                                  int m, int n, int k) {
    ...
    for (int bkIdx = 0; bkIdx < K; bkIdx += BK) {
        ...
        for (int i = threadIdx.x; i < BM * BK; i += blockDim.x) {
            ...
            int globalRow = cRow * BM + row;
            int globalCol = bkIdx + col;
        ...
        for (int i = threadIdx.x; i < BK * BN; i += blockDim.x) {
            ...
            int globalRow = bkIdx + row;
            int globalCol = cCol * BN + col;
...
template <int BM, int BN, int BK, int TM, int TN>
__global__ void matmul_at_b_kernel(float *A, float *B, float *C,
                                   int m, int n, int k) {
    ...
    for (int bkIdx = 0; bkIdx < K; bkIdx += BK) {
        ...
        for (int i = threadIdx.x; i < BM * BK; i += blockDim.x) {
            ...
            if (globalRow < M && globalCol < K)
                As[row * BK + col] = A[globalCol * n + globalRow]; // transpose
            else
                As[row * BK + col] = 0.0f;
        }
...
__global__ void bias_forward_kernel(float *x, float *bias, int batch_size, int size) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int b = idx / size; //gives batch index (row index) like 1,2,3,4
    int i = idx % size; // gives element within batch
...
void forward_timed(NeuralNetwork *nn, float *input, float *hidden, float *output, int batch_size, TimingStats *stats) {
    ...
    dim3 block_size((BN / TN) * (BM / TM));  // = 64 threads
    
    // Input to Hidden (X @ W1)
    clock_gettime(CLOCK_MONOTONIC, &start);
    dim3 grid_size1(CEIL_DIV(HIDDEN_SIZE, BN),CEIL_DIV(batch_size, BM));
    matmul_a_b_kernel<BM, BN, BK, TM, TN><<<grid_size1, block_size>>>(input, nn->weights1, hidden,batch_size, INPUT_SIZE, HIDDEN_SIZE);
    ...
    clock_gettime(CLOCK_MONOTONIC, &start);
    dim3 grid_size2(CEIL_DIV(OUTPUT_SIZE, BN),CEIL_DIV(batch_size, BM));
    matmul_a_b_kernel<BM, BN, BK, TM, TN><<<grid_size2, block_size>>>(hidden, nn->weights2, output,batch_size, HIDDEN_SIZE, OUTPUT_SIZE);
    CUDA_CHECK(cudaDeviceSynchronize());
    clock_gettime(CLOCK_MONOTONIC, &end);
...
void backward_timed(NeuralNetwork *nn, float *input, float *hidden, float *output, int *labels, int batch_size, TimingStats *stats) {
    ...
    clock_gettime(CLOCK_MONOTONIC, &start);
    dim3 grid_weights2(CEIL_DIV(OUTPUT_SIZE, BN),CEIL_DIV(HIDDEN_SIZE, BM));
    matmul_at_b_kernel<BM, BN, BK, TM, TN><<<grid_weights2, block_size>>>(hidden, grad_output, nn->grad_weights2,batch_size, HIDDEN_SIZE, OUTPUT_SIZE);
    CUDA_CHECK(cudaDeviceSynchronize());
    clock_gettime(CLOCK_MONOTONIC, &end);
    ...
    clock_gettime(CLOCK_MONOTONIC, &start);
    dim3 grid_weights1(CEIL_DIV(HIDDEN_SIZE, BN),CEIL_DIV(INPUT_SIZE, BM));
    matmul_at_b_kernel<BM, BN, BK, TM, TN><<<grid_weights1, block_size>>>(input, d_ReLU_out, nn->grad_weights1,batch_size, INPUT_SIZE, HIDDEN_SIZE);
    CUDA_CHECK(cudaDeviceSynchronize());
    clock_gettime(CLOCK_MONOTONIC, &end);
